...
class StockHelper:
    def __init__(self, rest_api: REST):
    ...
    def fetch_alpaca_data(
        self, symbol: str, start_date: str, end_date: str, tail: int
    ) -> pd.DataFrame:
        # Fetch historical data from Alpaca
        bars = (
            self.rest_api.get_bars(
...
class AvoAttributesCalculator:
    ...
    def calculate_action_points(num_samples, factor, first_derivative):
        ...
        gap_min = AvoAttributesCalculator.group_consecutives(min_)
        gap_max = AvoAttributesCalculator.group_consecutives(max_)
...
class CircleCalculator:
    ...
    def compute_circles(symbol, df, window, factor, current_price, df_temp):
        try:
            ...
            gradient, intercept = AvoAttributesCalculator.compute_avo_attributes(data_percent)

            data_filter, gradient, intercept = AvoAttributesCalculator.compute_trend_and_filter(
                num_samples, data_percent, window
            )

            velocity, first_derivative = AvoAttributesCalculator.calculate_velocity(data_filter)

            final_min, final_max, isamp, action = AvoAttributesCalculator.calculate_action_points(
